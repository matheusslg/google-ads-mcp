...
- **Developer-token approval delays for end users.** Google's current SLA for Basic Access review is ~3 business days; complex applications can take longer or be rejected without specific cause. End users may also stumble on the application form itself. Mitigation: ship a detailed `docs/developer-token.md` walking through every form field, common rejection patterns, and the Test-Account fallback for getting started immediately (no Basic Access required for development against synthetic data).
